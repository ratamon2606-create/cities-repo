...
class DB:
    ...
    def insert(self, file):
        self.db[file.table_name] = file.table
        return self.db

    def search(self, name):
        return Table(name, self.db[name])
...
class Table:
    ...
    def aggregate(self, function, key):
        try:
            value = [float(d[key]) for d in self.table if key in d]
        except ValueError:
            value = [d[key] for d in self.table if key in d]
        return function(value)
    
    def filter(self, condition):
        filtered = [d for d in self.table if condition(d)]
        return Table(self.table_name, filtered)
    
    def join(self, table, name):
        for i in self.table:
            for j in table.table:
                if i[name] == j[name]:
                    i.update(j)
        return Table(self.table_name, self.table)
